 ...
 *
 * Verbatim port of ui-ref-orii/frontend/src/pages/Landing.jsx. The only
 * adaptations from the reference:
 *   1. "use client" added (framer-motion + onClick handlers require it).
 *   2. react-router-dom navigation replaced with next/navigation router.push.
 *   3. The onLaunch callback (passed as prop in the reference SPA) is
 *      wired to router.push('/onboard') here — the existing onboarding
 *      surface handles wallet connection. All four launch buttons route
 *      to the same destination per the reference's intent.
 *
...
export default function Landing() {
  ...
  const onLaunch = () => router.push("/onboard")
  ...
  return (
    <main className="min-h-screen bg-white text-[#0A0A0A]" data-testid="landing-page">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8" data-testid="landing-nav">
        ...
        <Button onClick={onLaunch} className="rounded-none bg-black px-5 text-white hover:bg-[#0022FF]" data-testid="landing-connect-wallet-button">
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-5 pb-12 pt-8 sm:px-8 lg:grid-cols-12 lg:gap-12 lg:pb-16" data-testid="landing-hero-section">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="lg:col-span-7">
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row" data-testid="landing-cta-row">
            <Button onClick={onLaunch} className="rounded-none bg-[#0022FF] px-7 py-6 text-base text-white hover:bg-[#0019CC]" data-testid="landing-launch-app-button">
          ...
          <div className="mt-6 grid max-w-2xl grid-cols-1 gap-2 sm:grid-cols-3" data-testid="landing-login-options">
            <Button onClick={onLaunch} variant="outline" className="h-auto whitespace-normal rounded-none border-black/20 px-3 py-3 text-xs hover:bg-black hover:text-white" data-testid="login-email-privy-button"><Mail className="h-4 w-4" /> Email / Privy</Button>
            <Button onClick={onLaunch} variant="outline" className="h-auto whitespace-normal rounded-none border-black/20 px-3 py-3 text-xs hover:bg-black hover:text-white" data-testid="login-google-privy-button"><Sparkles className="h-4 w-4" /> Google / Privy</Button>
            <Button onClick={onLaunch} variant="outline" className="h-auto whitespace-normal rounded-none border-black/20 px-3 py-3 text-xs hover:bg-black hover:text-white" data-testid="login-metamask-button"><WalletCards className="h-4 w-4" /> MetaMask</Button>
          </div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }} className="relative border border-black/10 bg-[#F5F5F5] p-4 lg:col-span-5" data-testid="landing-hero-art-card">
      ...
      <section id="feature-grid" className="border-y border-black/10 bg-[#F5F5F5]" data-testid="landing-feature-section">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-px bg-black/10 px-5 py-px sm:px-8 md:grid-cols-2 lg:grid-cols-5">
          {featureTiles.map((tile, index) => {
            ...
            return (
              <motion.article key={tile.id} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="min-h-56 bg-white p-6" data-testid={`landing-feature-${tile.id}`}>
